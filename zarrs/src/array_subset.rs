...
impl Indexer for &[ArraySubset] {
    fn dimensionality(&self) -> usize {
        self.first().map(ArraySubset::dimensionality).unwrap_or(0)
    }

    fn len(&self) -> u64 {
        self.iter().map(ArraySubset::num_elements).sum()
    }
    ...
    fn iter_indices(&self) -> Box<dyn Iterator<Item = ArrayIndices>> {
        let indices = self
            .into_iter()
            .map(|array_subset| array_subset.indices())
            .collect::<Vec<_>>();
    ...
    fn iter_linearised_indices(
        ...
        let linearised_indices = self
            .into_iter()
            .map(|array_subset| array_subset.linearised_indices(array_shape))
    ...
    fn iter_contiguous_linearised_indices(
        ...
        let contiguous_linearised_indices = self
            .into_iter()
            .map(|array_subset| array_subset.contiguous_linearised_indices(array_shape))
            .collect::<Result<Vec<_>, _>>()?;
