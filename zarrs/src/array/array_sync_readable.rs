...
use super::{
    ...
    ArrayMetadataV3, DataType, DataTypeSize,
};
...
impl<TStorage: ?Sized + ReadableStorageTraits + 'static> Array<TStorage> {
    ...
    /// Helper method to retrieve multiple chunks with optional data types.
    fn retrieve_multi_chunk_optional(
        &self,
        array_subset: &ArraySubset,
        chunks: &ArraySubset,
        ...
        match num_chunks {
            ...
            _ => {
                ...
                if let DataType::Optional(inner_data_type) = chunk_representation.data_type() {
                    self.retrieve_multi_chunk_optional(
                        array_subset,
                        &chunks,
                        inner_data_type,
                        chunk_concurrent_limit,
