...
// Special ComplexFloat8E5M2 implementation with microfloat feature support
...
mod impl_microfloat {
    ...
    impl DataTypeTraits for ComplexFloat8E5M2DataType {
            ...
            if let Some([re, im]) = fill_value_metadata.as_array() {
                let parse_component = |v: &FillValueMetadata| -> Option<u8> {
                    if let Some(s) = v.as_str() {
                        ...
                        match s {
                            "NaN" => return Some(microfloat::f8e5m2::NAN.to_bits()),
                            "Infinity" => return Some(microfloat::f8e5m2::INFINITY.to_bits()),
                            "-Infinity" => {
                                return Some(microfloat::f8e5m2::NEG_INFINITY.to_bits());
                            }
                            _ => {}
                        }
                    }
                    if let Some(f) = v.as_f64() {
                        return Some(microfloat::f8e5m2::from_f64(f).to_bits());
                    }
                    None
                };
                if let (Some(re_byte), Some(im_byte)) = (parse_component(re), parse_component(im)) {
        ...
        fn metadata_fill_value(
            ...
            let component_to_metadata = |byte: u8| -> FillValueMetadata {
                let f8 = microfloat::f8e5m2::from_bits(byte);
                if f8.is_nan() {
                    FillValueMetadata::from("NaN".to_string())
