...
// Special Float8E4M3 implementation with microfloat feature support
    ...
    impl DataTypeTraits for Float8E4M3DataType {
        ...
        fn fill_value(
            ...
            if let Some(s) = fill_value_metadata.as_str() {
                ...
                match s {
                    "NaN" => return Ok(FillValue::from(microfloat::f8e4m3::NAN.to_bits())),
                    "Infinity" => {
                        return Ok(FillValue::from(microfloat::f8e4m3::INFINITY.to_bits()));
                    }
                    "-Infinity" => {
                        return Ok(FillValue::from(microfloat::f8e4m3::NEG_INFINITY.to_bits()));
                    }
                    _ => {}
                }
            }
            ...
            if let Some(f) = fill_value_metadata.as_f64() {
                return Ok(FillValue::from(microfloat::f8e4m3::from_f64(f).to_bits()));
            }
        ...
        fn metadata_fill_value(
            ...
            let bytes: [u8; 1] = fill_value
                ...
                .try_into()
                .map_err(|_| DataTypeFillValueError)?;
            let f8 = microfloat::f8e4m3::from_bits(bytes[0]);
...
// Special Float8E4M3 implementation with float8 and microfloat feature support
