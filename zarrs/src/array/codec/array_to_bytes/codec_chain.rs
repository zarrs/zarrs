...
mod tests {
    ...
    impl ArrayCodecTraits for LocalOnlyReshapeGridCodec {
        fn as_any(&self) -> &dyn std::any::Any {
            self
        }
    ...
    impl ArrayToArrayCodecTraits for LocalOnlyReshapeGridCodec {
    ...
    impl ArrayCodecTraits for TestSubchunkingCodec {
        fn as_any(&self) -> &dyn std::any::Any {
            self
        }
    ...
    impl ArrayToBytesCodecTraits for TestSubchunkingCodec {
        fn into_dyn(self: Arc<Self>) -> Arc<dyn ArrayToBytesCodecTraits> {
            self
        }
    ...
    fn local_only_grid_chain(expose_subchunks: bool, reject_chunk_local: bool) -> CodecChainBound {
        let data_type = data_type::uint8();
        let fill_value = FillValue::from(0u8);
        let mut array_to_array: Vec<Arc<dyn ArrayToArrayCodecTraits>> =
        ...
        CodecChainBound {
            array_to_array,
            array_to_bytes: Arc::new(TestSubchunkingCodec {
                data_type,
                fill_value,
                expose_subchunks,
            }),
