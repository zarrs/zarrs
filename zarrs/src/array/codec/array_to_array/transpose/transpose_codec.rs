...
use crate::array::chunk_grid::{
    ChunkEdgeLengths, RectilinearChunkGrid, UnstructuredCartesianChunkGrid,
};
    ...
    fn encoded_chunk_grid(
        ...
        for &decoded_dim in &self.order.0 {
            let Some(edge_lengths) = decoded_chunk_grid.chunk_edge_lengths(decoded_dim)? else {
                return Ok(ChunkGridEncoded::Array(permuted_unstructured_chunk_grid(
                    decoded_chunk_grid,
                    array_shape,
                    &self.order.0,
                )?));
    ...
    fn decoded_subchunk_grid(
        ...
        for &encoded_dim in &inverse {
            let Some(edge_lengths) = encoded_subchunk_grid.chunk_edge_lengths(encoded_dim)? else {
                return Ok(ChunkGridDecoded::Array(permuted_unstructured_chunk_grid(
