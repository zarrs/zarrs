...
impl DataType {
    ...
    pub fn size(&self) -> DataTypeSize {
        match self {
            ...
            Self::Optional(inner) => match inner.size() {
                DataTypeSize::Fixed(size) => DataTypeSize::Fixed(size + 1),
                DataTypeSize::Variable => DataTypeSize::Variable,
            },
...
mod tests {
    ...
    fn data_type_optional() {
        ...
        assert_eq!(data_type.size(), DataTypeSize::Fixed(5)); // 4 bytes + 1 validity byte
