...
/// An asynchronous cache for shard indexes with methods for efficiently reading inner chunks of a sharded array.
pub struct AsyncArrayShardIndexCache<'a, TStorage: ?Sized> {
    array: &'a Array<TStorage>,
    cache: Arc<async_lock::Mutex<PartialDecoderHashMap>>,
}

impl<'a, TStorage: ?Sized> AsyncArrayShardIndexCache<'a, TStorage> {
...
impl<TStorage: AsyncReadableStorageTraits + 'static> AsyncArrayShardIndexCache<'_, TStorage> {
    async fn retrieve(
        &self,
        shard_indices: &[u64],
    ...
    pub async fn async_inner_chunk_byte_range(
        &self,
        ...
        if self.array.is_exclusively_sharded() {
            ...
            let partial_decoder = self.retrieve(&shard_indices).await?;
    ...
    pub async fn async_retrieve_encoded_inner_chunk(
        &self,
        inner_chunk_indices: &[u64],
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_inner_chunk_opt(
        &self,
        inner_chunk_indices: &[u64],
        options: &CodecOptions,
        ...
        if self.array.is_sharded() {
            ...
            self.array
                .async_retrieve_chunk_opt(inner_chunk_indices, options)
                .await
        }
    }
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_inner_chunk_elements_opt<T: ElementOwned + Send + Sync>(
        &self,
        inner_chunk_indices: &[u64],
        options: &CodecOptions,
        ...
        T::from_array_bytes(
            ...
            self.async_retrieve_inner_chunk_opt(inner_chunk_indices, options)
                .await?,
        )
    }
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_inner_chunk_ndarray_opt<T: ElementOwned + Send + Sync>(
        &self,
        inner_chunk_indices: &[u64],
        options: &CodecOptions,
        ...
        if let Some(inner_chunk_shape) = self.array.inner_chunk_shape() {
            super::elements_to_ndarray(
                ...
                self.async_retrieve_inner_chunk_elements_opt::<T>(inner_chunk_indices, options)
                    .await?,
            )
        } else {
            self.array
                .async_retrieve_chunk_ndarray_opt(inner_chunk_indices, options)
                .await
        }
    }
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_inner_chunks_opt(
        &self,
        ...
        if self.array.is_sharded() {
            ...
            self.async_retrieve_array_subset_sharded_opt(&array_subset, options)
                .await
        } else {
            self.array
                .async_retrieve_chunks_opt(inner_chunks, options)
                .await
        }
    }
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_inner_chunks_elements_opt<T: ElementOwned + Send + Sync>(
        &self,
        inner_chunks: &ArraySubset,
        options: &CodecOptions,
    ) -> Result<Vec<T>, ArrayError> {
        T::from_array_bytes(
            self.array.data_type(),
            self.async_retrieve_inner_chunks_opt(inner_chunks, options)
                .await?,
        )
    }
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_inner_chunks_ndarray_opt<T: ElementOwned + Send + Sync>(
        &self,
        inner_chunks: &ArraySubset,
        options: &CodecOptions,
    ) -> Result<ndarray::ArrayD<T>, ArrayError> {
        ...
        let elements = self
            .async_retrieve_inner_chunks_elements_opt::<T>(inner_chunks, options)
            .await?;
    ...
    pub async fn async_retrieve_array_subset_sharded_opt(
        &self,
        array_subset: &ArraySubset,
        options: &CodecOptions,
    ) -> Result<ArrayBytes<'_>, ArrayError> {
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_array_subset_elements_sharded_opt<T: ElementOwned + Send + Sync>(
        &self,
        array_subset: &ArraySubset,
        options: &CodecOptions,
    ) -> Result<Vec<T>, ArrayError> {
    ...
    #[allow(clippy::missing_errors_doc)]
    pub async fn async_retrieve_array_subset_ndarray_sharded_opt<T: ElementOwned + Send + Sync>(
        &self,
        array_subset: &ArraySubset,
        options: &CodecOptions,
    ) -> Result<ndarray::ArrayD<T>, ArrayError> {
        let elements = self
            .async_retrieve_array_subset_elements_sharded_opt::<T>(array_subset, options)
            .await?;
        super::elements_to_ndarray(array_subset.shape(), elements)
    }
}
...
mod tests {
    ...
    async fn array_sharded_ext_impl(sharded: bool) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let cache = AsyncArrayShardIndexCache::new(&array);
        ...
        if sharded {
            ...
            let compare = array
                ...
                .await?;
            let test = cache
                .async_retrieve_inner_chunk_elements_opt::<u16>(&[2, 3], &CodecOptions::default())
                .await?;
            ...
            let encoded_inner_chunk = cache
                .async_retrieve_encoded_inner_chunk(&[0, 0])
                .await?
                .unwrap();
            assert_eq!(
                cache
                    .async_inner_chunk_byte_range(&[0, 0])
                    .await?
                    .unwrap()
                    .length(u64::MAX),
            ...
            let compare = array
                ...
                .await?;
            let test = cache
                .async_retrieve_inner_chunk_elements_opt::<u16>(&[1, 1], &CodecOptions::default())
                .await?;
            assert_eq!(compare, test);
            ...
            assert!(cache
                ...
                .await
                .is_err());
            assert!(cache.async_inner_chunk_byte_range(&[0, 0]).await.is_err());
        }
