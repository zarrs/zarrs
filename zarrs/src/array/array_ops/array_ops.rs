...
pub trait ArrayOps {
    ...
    /// Return the number of subchunk grid levels.
    #[must_use]
    fn num_subchunk_grid_levels(&self) -> usize;

    /// Return the direct subchunk grid of the outermost `sharding_indexed` codec.
    ///
    ...
    ///
    /// Level zero is the direct subchunk grid of the outermost `sharding_indexed` codec.
    /// Subsequent levels correspond to nested `sharding_indexed` codecs, outermost first.
    #[must_use]
