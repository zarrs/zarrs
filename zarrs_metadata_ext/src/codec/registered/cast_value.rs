...
pub struct CastValueCodecConfigurationV1 {
    /// The data type that values are cast to during encoding.
    pub data_type: MetadataV3,
    /// The rounding mode used when an exact cast is not possible.
    #[serde(default)]
    pub rounding: CastValueRoundingMode,
...
pub struct CastValueScalarMap {
    ...
    #[serde(default)]
    pub encode: Vec<[FillValueMetadata; 2]>,
    /// Exact mappings applied during decode.
    #[serde(default)]
    pub decode: Vec<[FillValueMetadata; 2]>,
}
...
mod tests {
    ...
    fn codec_cast_value_config_defaults() {
        ...
        assert_eq!(config.rounding, CastValueRoundingMode::NearestEven);
